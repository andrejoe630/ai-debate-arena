import type { SavedDebate, ModelStats, ModelKey, DebateResultV2, DiscussionResult, Mode } from './types'
...
export function saveDebate(mode: Mode, result: DebateResultV2 | DiscussionResult): string {
  ...
  const savedDebate: SavedDebate = {
    ...
    mode,
    timestamp,
    ...(mode === 'debate' ? { debateResult: { ...result as DebateResultV2, id, timestamp } } : { discussionResult: { ...result as DiscussionResult, id, timestamp } })
  }
...
export function getModelStats(): Record<ModelKey, ModelStats> {
  ...
  return {
    ...
    anthropic: { model: 'anthropic', wins: 0, losses: 0, ties: 0, totalDebates: 0 },
...
export function downloadFile(content: string, filename: string, type: string = 'text/markdown'): void {
